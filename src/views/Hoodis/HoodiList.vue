<template>
    <div>
      ...
      <div class="item-box" v-for="item in Hoodis" :key="item.id">
        <div class="img-box">
        ...
        <div class="txt-box">
          ...
          <router-link class="nav-link" :to="'/Hoodis/' + item.id">{{
            item.title
          }}</router-link>
  ...
  export default {
    ...
    methods: {
      fetchItems() {
        axios.get("http://localhost:3002/Hoodis").then((response) => {
          this.Hoodis= response.data;
        });
      },
      editItem(id) {
      ...
      deleteItem(id) {
        axios.delete(`http://localhost:3002/Hoodis/${id}`).then(() => {
          this.fetchItems();
        });
      },
      newItem() {
